...
- 直近の見直し: 2026-08-11（オントロジー / OKF の高優先2件を実装して畳んだ —
  [B-41](#b-41) が `ontology.yaml` に `okf:` 節を立て、[B-40](#b-40) がデッキ名の綴りを
  1つに揃えた。宣言の版は 4 → 5）
- その前: 2026-08-10（B-38〜B-43 を追加して全体を並べ替え、[B-44](#b-44) を見つけてその場で直した。
...
| デッキ slug の衝突（`My_Deck.md` と `my-deck.md`） | ✅ `--wiki` が exit 1 で両方のファイル名を挙げる → 畳んだ [B-40](#b-40) |
...
まず起動時間（[B-25](#b-25) の周辺）を見るのが順序として正しい。
...
1つも宣言されていない」と書いていた。**その穴は塞がった**（畳んだ [B-41](#b-41) が
`okf:` 節を立て、[B-40](#b-40) が実装をそこに合わせた）。宣言が無い層では lint も
...
[B-40](#b-40) はその逆側の実例で、デッキ slug は**デッキをまたぐ**ため lint には見えず、
...
デッキが**どこにも書かれていない名前**になる（畳んだ [B-40](#b-40)）。どれも exit 0 で、lint も緑である。
...
（このあと [B-41](#b-41) と [B-40](#b-40) は実装して[完了](#done)へ畳んだ。）
...
| [B-29](#b-29) | P1 → **P2** | コアの宣言駆動化。読み書きする人には見えず、[B-41](#b-41) で宣言を広げたあとに形が決まる |
...
> （**書き方を1つに絞ると、規則のほうが要らなくなる**）。続く回で [B-41](#b-41) が
...
（17レイアウト × スロット、+ selfcheck で嘘を防ぐ）。畳んだ [B-41](#b-41) と同じ種類・同じ規模で、
...
- **コード中の `BACKLOG B-NN` 参照を誰も照合していない**（10箇所ほどある）。
  実例: [B-25](#b-25) の作業で `ci.yml` に書いた残件の参照が、直後に項目を分けた時点で
...
**一段悪い形が1件見つかった（[B-41](#b-41) の作業中）。** `inline.syntaxes[].guidance` は
...
（[B-32](#b-32) が数え直した3回のうち、解決表のぶん）。**ただしこれは動機にならない** —
...
[B-32](#b-32) が数えている「存在しない保証の主張」の実例が、これで1件減る。
...
`ontology.md` に一度も出ていなかった** — [B-28](#b-28) の「誰も読まない宣言」の中でも、
