...
**このファイルは現役の項目だけを持つ。** 手が離れたものは別ファイルにある —
畳んだ項目は [BACKLOG-DONE.md](BACKLOG-DONE.md)、やらないと決めたものは
[BACKLOG-WONTDO.md](BACKLOG-WONTDO.md)。**ID は3ファイルで1つの空間**で、再利用しない
...
| **あとでやる** | **P2** | 読み書きする人には見えないが、直さないと**上の band が繰り返し生えてくる**土台 |
| | **P3** | 主題の外。やっても Wiki は良くならない（が、捨てるほどでもない） |
| **やらない** | — | 落とした理由ごと [BACKLOG-WONTDO.md](BACKLOG-WONTDO.md) に置く。ID は再利用しない |
| **完了済み** | — | 畳んだ要点を [BACKLOG-DONE.md](BACKLOG-DONE.md) に置く。コードの `BACKLOG B-NN` の行き先 |

**区分は band の上に被せた粗い線で、band を置き換えるものではない。** 着手の順は band が持ち、
区分が持つのは「いま読むべきか」だけ — このファイルを開いた人が
[すぐやった方がよい](#すぐやった方がよい)の9件だけを見て手を動かせるようにするためにある。
...
このファイルには置かない。**やらない**は [BACKLOG-WONTDO.md](BACKLOG-WONTDO.md)（B-10 / B-11 /
B-16 / B-17 / B-18 と、落とした理由）、**完了済み**は [BACKLOG-DONE.md](BACKLOG-DONE.md)
（B-01 / B-02 / B-08 / B-14 / B-24 / B-25 / B-38 / B-40 / B-41 / B-44 / B-46 の要点）。
...
| [B-28](#b-28) | P1 → **P2** | 「どの宣言が規範か」を可視化する項目。[B-39](#b-39) と [B-41](BACKLOG-DONE.md#b-41) が具体例のほうを先に片付けるので、**残りが減ってから**やるほうが安い |
| [B-29](#b-29) | P1 → **P2** | コアの宣言駆動化。読み書きする人には見えず、[B-41](BACKLOG-DONE.md#b-41) で宣言を広げたあとに形が決まる |
