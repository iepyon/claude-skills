...
- 直近の見直し: 2026-08-10（B-38〜B-43 を追加して全体を並べ替え、[B-44](#b-44) を見つけてその場で直した。
  そのあと記述を実測で突き合わせ、[B-38](#b-38) を実装して畳んだ）
...
| `npx tsc --noEmit` | ❌ 3 件（`schema/theme.ts:236,240,262`）→ [B-25](#b-25) |
...
| [B-25](#b-25) | P1 | 型検査が CI にもテストにも無い | ツール品質 |
...
（[B-32](#b-32) / [B-25](#b-25) / [B-07](#b-07)）。P0 と違って一本の層ではないが、
