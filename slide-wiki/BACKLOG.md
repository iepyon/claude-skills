...
デッキ名・ID の一意性といった**サイトの階層は1つも宣言されていない**こと
...
| ID | 移動 | 理由 |
|---|---|---|
| [B-41](#b-41) | P1 → **P0** | 「宣言が正本」を掲げる道具で、**主題そのものが宣言されていない**。実害も具体的で、予約ファイル名やデッキの並べ方が `ontology.md` に無く、コードと4箇所の散文にしか無い。**変えたら直し忘れが検出されない** |
...
Wiki を主題にした以上、この道具の看板はリンクである。その層が、**宣言が足りず**（[B-41](#b-41)）、
**半分しか実装されておらず**（[B-22](#b-22)）、**ほとんど何も報告しない**（[B-39](#b-39) / [B-36](#b-36)）。
4件は別々の不具合ではなく、同じ1つの層の3つの面である。

> **記法を OKF v0.2 に移した回で、この band は一段軽くなった。** 内部リンクが
> バンドル相対の絶対パス1本になり、4段階の解決順そのものが消えたので、
> [B-41](#b-41) が数えていた「3箇所に手書きされた解決順」は解決順ごと無くなった
> （残るのはデッキ集合と `order.yaml` の宣言）。[B-40](#b-40) も半分閉じている。
> **書き方を1つに絞ると、規則のほうが要らなくなる**という畳み方である。
...
**一部解消済み（2026-08-11）**: 参照の形と解決順は宣言に入った。`inline.syntaxes` の
`internal-link` が `[ラベル](/デッキ名.md#スライドID)` を綴り、絞った理由と OKF との差
（相対パスと裸の `#id` は解決しない／`#id` は OKF の規定外）も `guidance` が持つ。
**解決順の宣言は要らなくなった** — 段が1つしか無いため。以下は残っている部分。
...
`order.yaml`、ID がデッキ内で一意であること、OKF の予約ファイル名 —
は**1つも宣言されていない**（予約名の正本は `src/okf.ts` というコードの中にある）。
...
作られている点である。予約ファイル名（`index.md` / `log.md`）は `src/okf.ts`・`SKILL.md`・
`CLAUDE.md`・`guide` デッキの4箇所に別々に書かれていて、どれも互いを照合していない。
**変えたら4箇所を手で直すことになり、直し忘れは検出されない。** これは [B-01](#b-01) が
md の構造について潰した状態そのものが、サイトの層でまだ残っている、ということである。
...
**実装方針**: `ontology.yaml` に `okf:` を足す（バンドルの根・予約ファイル名・
デッキ集合と `order.yaml`・deck-slug の作り方・ID の一意性の範囲・`okf_version`）。
`CONSUMED_KEYS` に足して `gen-ontology-doc.ts` に `ontology.md` へ出させ、
SKILL.md にも生成領域を作る。`ontology.test.ts` が宣言の予約名と `src/okf.ts` の
`RESERVED_OKF_FILES` を照合してドリフトを赤くする。

**受け入れ基準**: `ontology.md` を読んだだけで、バンドルに置いてよいファイル名と
デッキの並べ方が分かる。予約名を1つ増やすと SKILL.md の生成領域が更新され、
コードとの食い違いでテストが赤くなる。
...
**背景**: 「ファイル名がデッキの slug であり `/deck.md#slide` のリンク先でもある」は
...
つまり `My_Deck.md` は、`order.yaml` には `My_Deck` と書き、`/My_Deck.md#…` と指すことになる。
**同じものを指す2つの綴りが、どちらのドキュメントにも書かれていない。**

**一部解消済み（2026-08-11）**: リンク側は閉じた。参照が実ファイル名を綴るようになり、
`okf.ts` の `parseOkfLink` がそのパスに `slugify` を掛けて slug に直すので、
**リンクに書く名前とファイル名は定義上一致する**。`migrate-wikilinks.ts` は
slug が衝突する2ファイルを見つけたら終了コード 2 で止める。残っているのは
`order.yaml` の照合（生のファイル名）と `--wiki` 本体の衝突検出。
...
**実装方針**: `orderDeckFiles` の照合を slug に揃える。`buildSiteIndex` の
デッキ slug の衝突は連番にせず誤りとして止める（スライド ID と違い、デッキはファイルなので
書き手が改名できる）。判定は `migrate-wikilinks.ts` が既に持っているものを本体へ移す。
...
**リンクをバンドル相対の絶対パス1本に絞った時点で、解決はデッキに依らなくなっている**
