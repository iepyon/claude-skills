...
食い違い — 明示 ID の衝突（[B-38](#b-38)）、効かない場所のリンク（[B-39](#b-39)）、
デッキ名の二重の語彙（[B-40](#b-40)）— は**どれも緑のまま通る**。
...
リンクが生の文字で出る（[B-22](#b-22)）、リンクが**別のスライドに繋がる**（[B-38](#b-38)）、
...
| [B-38](#b-38) | P0 | 明示した ID の衝突が黙って別の ID になる | Wiki |
...
**半分しか実装されておらず**（[B-22](#b-22)）、**何も報告しない**（[B-38](#b-38) / [B-39](#b-39) / [B-36](#b-36)）。
5件は別々の不具合ではなく、同じ1つの層の3つの面である。

**着手の順は重要度の順ではない。** 先にやるのは報告の3件（[B-39](#b-39) → [B-38](#b-38) → [B-36](#b-36)）で、
どれも lint と終了コードに1つずつ足すだけの作業。これを先に入れると
...
同じ理由で、[B-38](#b-38) / [B-39](#b-39) / [B-40](#b-40) はどれも
「宣言が無いので検査を立てる足場が無い」形をしている。個別に直すこともできるが、
**宣言を1段広げるほうが、その3つが同じ場所から出てくる。**
...
<a id="b-38"></a>
### B-38: 明示した ID の衝突が黙って別の ID になる

**背景**: `assignSlideIds`（`parser/slide-ids.ts`）は、同じ ID が二度要求されたら
`unique()` で連番を足す。**この処理が、自動生成された slug と `<!--id:-->` で明示された ID を
区別しない。** 書き手が2枚のスライドに `<!--id:seed-->` と書くと、2枚目は黙って `seed-2` になる。

自動 slug の衝突（同じタイトルのスライドが2枚）を連番で捌くのは妥当だが、
**明示 ID は書き手の意思表示である。** 衝突は「どちらかが間違っている」のサインであって、
機械が勝手に一意化してよいものではない。しかも折れたリンクと違って**何も赤くならない**:

| 確認 | 結果 |
|---|---|
| `--lint --strict` | ✅ 「宣言に沿っている」 |
| `--wiki` | ✅ exit 0、未解決リンク 0 件 |
| サイトの ID | `dup/seed` と `dup/seed-2` |

**これは折れたリンクより悪い。** 折れたリンクはサイドバーに出て、直せば消える。
`[[seed]]` は**解決する** — 常に1枚目へ。2枚目は誰からも指されないスライドとして
サイトに残り、書き手は繋いだつもりでいる。壊れ方が「動いているように見える」側に倒れている。

`slide-ids.ts` のコメントが「**明示 ID の衝突も黙って解決せず**、あとで /lint 相当の検出が
できるよう元の並び順は保つ」と書いているが、**黙って解決している**し、その /lint 相当は
存在しない（[B-32](#b-32) と同じ、実在しない保証を主張するコメント）。

**実装方針**: `assignSlideIds` が明示 ID の衝突を診断として返し、lint が拾う。
自動 slug の衝突は現状どおり連番のまま（そちらは連番が正しい振る舞い）。
ID は**デッキ内で**採番されるので検査もデッキ内で閉じる — トークン層に `<!--id:-->` が
2度現れることを見れば足り、`lint.ts` が既に立っている場所でそのまま書ける。

**受け入れ基準**: 同じ `<!--id:-->` を2枚に書くと `--lint` が両方の行番号つきで報告し、
`--strict` で落ちる。タイトルが同じだけの2枚は引き続き連番で通る。
...
作業で、着手までの間ずっとこの穴が開いている。**宣言は既にあるので、検査を足すのは安い。**
効く場所の宣言を lint に読ませるだけで、直すまでの間「そこには書けない」と言える。

**実装方針**: `lint.ts` に `inline.effective-in` を読ませ、効かない場所のトークンに
`[[…]]`（および `[…](…)`）が現れたら「このレイアウトのここでは リンクにならない」と報告する。
判定に要るのは「いまのレイアウトのどの slot か」だけで、lint は既にそれを持っている
（未宣言 `###` の検査で使っている）。B-22 を直したらレイアウトごとに対象から外れていく
— つまり**この診断は B-22 の進捗メータにもなる**。

**受け入れ基準**: table のセルに書いた `[[…]]` が `--lint` で報告される。
`###` 本文に書いたものは報告されない。B-22 で対応済みの agenda も報告されない。
...
- `parser/slide-ids.ts` の `assignSlideIds` のコメントが「明示 ID の衝突も黙って解決せず、
  あとで /lint 相当の検出ができるよう元の並び順は保つ」と書くが、**黙って連番で解決している**し、
  その /lint 相当も無い（[B-38](#b-38)）。「保つ」と言っている並び順のほうは実際に保たれているので、
  誤っているのは前半だけ — 直すのは B-38 の作業と同時になる
