...
| [B-48](#b-48) | P2 | 型の付いた関係がサイトに出ていない | Wiki |
| [B-49](#b-49) | P2 | 関係の型付けが patterns-meta で止まっている | Wiki |
...
### B-48: 型の付いた関係がサイトに出ていない

**背景**: `relations.yaml` の辺は今、lint と宣言の中にしか無い。読み手が見るサイトは
無型のバックリンク（`link-graph.ts` の `string → string[]`）のままで、
「この2枚は対である」「こちらが上位である」は画面に出ていない。

**実装方針**: `renderer/wiki/` に型つきの欄を足す。**`.slide` の中に入れてはいけない** —
DOM が変われば `--html` と PPTX の3者比較が割れるし、そもそも **関連は文中に溶かす**ので
パターン本体に「関連:」の欄を作る形にはできない。置き場はバッジと同じ
`.wiki-slide` の中・`.slide` の兄弟で、CSS は `wiki/styles.ts` にだけ書く。

`LinkGraph` は無型のまま残す（リンク切れの検出という別の仕事を持っている）。
型つきの辺は別の構造として渡し、テンプレートが両方を読む。

### B-49: 関係の型付けが patterns-meta で止まっている

**背景**: `relations.yaml` の `decks:` は `patterns-meta.md` の1本だけ。
`patterns-wiki.md` の10枚と、デッキをまたぐ辺（`3ストライクで書く` → `種ノート` など）は
まだ型を持たない。またぐリンクは `prose.exclude` の `cross-deck` で分類を免除している。

**実装方針**: `patterns-wiki.md` を `decks:` に足して、本文の散文リンクを型付けする。
またぐ辺はそのあと — **言語内のリンクと言語間のマッピングは別の語彙にする**
（SKOS が `skos:broader` と `skos:broadMatch` を分けたのと同じ理由で、
「同じ言語の中で上下に噛み合う」と「別の言語の似た概念を指す」は違うことを言っている）。

**先に決めることが1つある**: いま上下の軸を持たないパターンが3枚ある
（`3ストライクで書く`・`声に出して読む`・`黙って聴く著者`。`relation-coverage` が報せる）。
門と仕上げのパターンは大きさの軸ではなく手順の軸で噛んでいるので、
**`不揃いの石畳` の「3方向へ噛ませる」をこの3枚にも求めるのか**を決めないと、
デッキを増やしたときに同じ形の警告が増えるだけになる。
