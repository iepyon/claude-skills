...
- `kpi` — 効くのは Dashboard レイアウトの中だけ。他のレイアウトに書くと黙って捨てられる （lint が `annotation-scope` で報告する）。`<!--chart:-->` とは同居できない。

- `chart` — 効くのは Dashboard レイアウトの中だけ。種類は bar / line / donut。 系列は1つだけ（表は「ラベル \| 値」の2列。3列以上は変換が止まる）。 `<!--kpi-->` とは同居できない。
...
- ディレクティブ: `<!--dashboard:1,3,1-->`
- `_tag`: `Dashboard`（プラグイン `dashboard`）
- 説明: 行ごとに列数を指定するグリッドに、KPI タイルとグラフを並べるダッシュボード
- 効く注釈: `id`・`kpi`・`chart`
- 文字数上限: **400**（既定の上書き）

引数は行ごとの列数（`1,3,1` = 1列・3列・1列の3行）。`###` セルは書いた順に
左上から行優先で埋まり、**件数は列数の合計と一致させる**（違うと変換が止まる）。
セルの種類は注釈で決まる: `<!--kpi-->` は KPI タイル、`<!--chart:bar|line|donut-->` は
グラフ、注釈なしは見出し+本文のテキストセル。

KPI タイルは本文1行目が値。**数字は大きく、通貨記号や単位は小さく**描かれる
（`¥1.2億` は ¥ と億が数字の 55% の大きさになる）。2行目（任意）は前期比で、
先頭の記号で色が決まる（`+` `▲` `↑` は緑、`-` `▼` `↓` は赤）。続けて
`- <数値>` の箇条書きを置くと、スパークライン（小さな推移線）になる。

グラフのデータはパイプ区切りの表（1列目=ラベル、2列目=値）。ヘッダ行は
描画されず、系列は1つだけ（3列以上の表は変換が止まる）。グラフは SVG として
描かれるので、PPTX 上では画像になる（PowerPoint での編集は不可）。

文字数に数えるのは見出し・本文・KPI の値と前期比・グラフのラベルで、
表の数値とスパークラインの数値は数えない（データの密度はグラフの可読性が
上限であって、散文の上限ではない）。

| スロット | 記号 | 個数 | 見出し | 本文 | 説明 |
|---|---|---|---|---|---|
| `cells` | `###` | `1..n` | 自由 | `free` | セル。件数は列数の合計に一致させる（合わないと変換が止まる）。 |
