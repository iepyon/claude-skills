...
名前が同じなので、[[語彙を寄せる|寄せ先]] は1つに見える。

**名前が同じでも、指すものが同じとは限らない。**
寄せた一枚は、どちらの部署からも半分外れる。
AI は文脈を見ないので、名前が合えば寄せる。

### そこで
**文脈が違うなら、同じ名前でも別の語として扱う。**
語彙表は文脈ごとに持つ。混ざったら [[patterns-wiki/一枚一義|割る]]。
寄せるか割るかを決めるのは人。[[判子]] の内側。

![同姓同名](diagrams/patterns-llm-wiki/同姓同名.svg)
