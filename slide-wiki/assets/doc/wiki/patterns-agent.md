...
<!--id:厨房ごと渡す-例-->
<!--grid:3x1-->
### Codex の harness（OpenAI）
5か月で約100万行の製品を、人の手書きゼロで出した。人が書いたのはリポジトリ構成と CI と整形規則と linter で、コードそのものではない。
### Claude Science（Anthropic）
60を超える科学データベースと解析ツールを1つの作業台に集める。エージェントは道具を探さずに済み、探せない道具は最初から使われない。
### 一人起業
残るのは自前の手順と溜めたデータと連携で、頼み方の巧さではない。使うたびに厚くなるので、同じ道具を持つ他人にも真似しにくくなる。
<!--takeaway-->
...
### 一人起業
問題、需要の確認、先に売る、MVP、自動化の順に進める。作ってから売れるかを見ない。最初の関門が、いちばん安い関門でもある。
<!--takeaway-->
...
### 一人起業
小さく多く出し、需要の出た1つだけ残す。外れが安いほど打てる回数が増え、1回に賭けると外したときの次の弾が無い。
<!--takeaway-->
数が効くのは、篩が固いときだけである。
---
...
<!--grid:3x1-->
### Virtual Lab（Stanford）
PI 役のエージェントが、化学者と計算機科学者と批評役を率いて研究会を回す。議題を配るのは座長で、人が返すのは高次のフィードバックだけ。
### Claude Science（Anthropic）
調整役エージェントが専門サブエージェントに仕事を配り、別立ての査読役が引用と計算を検める。束ねる役と検める役を、別々に立てている。
### C コンパイラ（Anthropic）
逆に座長を置かなかった。各自が Git にロックを書いて持ち場を取り、2体が同じ物を取ると、マージ衝突が2体目に別を取れと告げる。
<!--takeaway-->
...
<!--grid:3x1-->
### Virtual Lab（Stanford）
批評役エージェントをチームに常置する。賛成する専門家だけを並べると会議は速く進むが、誰も止めない。反対する口が席にあること自体が仕掛けになる。
### Claude Science（Anthropic）
査読役エージェントが引用と計算をすべて照合し、見つけた誤りをその場で直す。書いた本人ではないので、通した理由をなぞらずに済む。
### Codex（OpenAI）
まずローカルで自分の変更を見直させ、そのうえで別のレビュー役を指名して呼ぶ。人の指摘にもエージェントの指摘にも応え、全員が納得するまで往復する。
<!--takeaway-->
...
<!--grid:3x1-->
### 一人起業
自己申告と24時間後の実際を突き合わせる日次バッチを回す。報告では済んだことになっているのに何も起きていない、という失敗が捕まる。
### 研究の引用
2026年の最初の7週で、277本に1本が捏造参考文献を含んだ。前年は458本に1本。書式も所属も日付も正しく見えるので、読んだだけでは分からない。
### 生成コード
完全には信頼しない開発者が96%。それでもコミット前に必ず検証するのは48%。もっともらしいものは、流し読みで通ってしまう。
<!--takeaway-->
...
<!--grid:3x1-->
### 長時間の開発（Anthropic）
要約して詰め直す圧縮だけでは、長い仕事に足りなかった。セッションを畳み、構造化された引き継ぎファイルから組み直す全面リセットが要る。
### Kosmos（Edison Scientific）
構造化された世界モデルを持ち、1回の走行に1,500本の論文と42,000行の解析コードを通す。覚えているのではなく、外に書いてあるから続けられる。
### 一人起業
走らせる前に、何をさせたいかのログを1本置く。5行のコードで、失敗のおよそ30%が捕まる。
<!--takeaway-->
覚えさせるのではなく、書いて置いていく。
---
...
<!--grid:3x1-->
### Robin（FutureHouse）
加齢黄斑変性の治療仮説を2.5か月で立て、検証した。仮説も実験設計も解析も図もエージェントが出したが、物理実験を回したのは人の研究者だった。
### Virtual Lab（Stanford）
人が返すのは高次のフィードバックだけ。問いを立てる場所と、結果を受け取る場所に人が残る。その2点以外は渡してよい、という線引きになる。
### 一人起業
顧客に届くものは、レビューを通さずに出さない。届いた後では取り消せないので、押す前に見る。
<!--takeaway-->
