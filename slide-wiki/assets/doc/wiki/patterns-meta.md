...
### 効き: [ジワる名前](patterns-meta.md#ジワる名前)
### 借りる: [メタファ](patterns-meta.md#メタファ)
### 拾う: [ドメイン言語](patterns-meta.md#ドメイン言語)
### 対: [往復切符](patterns-meta.md#往復切符)
### 検算: [声に出して読む](patterns-meta.md#声に出して読む)
### 次: [振る舞い](patterns-meta.md#振る舞い)
### 目次: [読み方](patterns-meta.md#読み方)
...
### 証拠: [3ストライクで書く](patterns-meta.md#3ストライクで書く)
### 力: [ザワつく状況](patterns-meta.md#ザワつく状況)
### 選ぶ: [ときめく解法](patterns-meta.md#ときめく解法)
### 生む: [たのしい釣り方](patterns-meta.md#たのしい釣り方)
### 図: [ラフで出す](patterns-meta.md#ラフで出す)
### 次: [仕上げ](patterns-meta.md#仕上げ)
### 目次: [読み方](patterns-meta.md#読み方)
...
### 検算: [声に出して読む](patterns-meta.md#声に出して読む)
### 合評: [黙って聴く著者](patterns-meta.md#黙って聴く著者)
### 次: [繋がり](patterns-meta.md#繋がり)
### 目次: [読み方](patterns-meta.md#読み方)
...
### 導く: [文に溶かす](patterns-meta.md#文に溶かす)
### 繋ぐ: [不揃いの石畳](patterns-meta.md#不揃いの石畳)
### 対: [往復切符](patterns-meta.md#往復切符)
### 検算: [声に出して読む](patterns-meta.md#声に出して読む)
...
理屈で選ぶと薄れる。[声に出して読む](patterns-meta.md#声に出して読む) と効いてくる。
説明は本文と [メタファ](patterns-meta.md#メタファ) が引き受ける。
...
### そこで
**すでに全員が知っている比喩を借りる。**
借り物なら手間はゼロ。[ジワる名前](patterns-meta.md#ジワる名前) になる。
説明が要る比喩なら、[ドメイン言語](patterns-meta.md#ドメイン言語) に替える。
...
<!--pattern-->
### いつ・なにが困るか
[メタファ](patterns-meta.md#メタファ) を探したが、どれも少しずつずれる。
...
**ドメイン専門家が使う語彙を、全員で使う。**
作るのではなく、聞いて拾う。手間は残る。
定義は一箇所に置き、[声に出して読む](patterns-meta.md#声に出して読む)。
...
<!--pattern-->
### いつ・なにが困るか
効いた打ち手に [ジワる名前](patterns-meta.md#ジワる名前) を付け、一枚にする。
行きの名前があれば、それで足りると思う。

**逆が書けない解は、パターンではなくべき論。**
「使わない」という判断が、読み手から消える。
一方通行だけでは、[不揃いの石畳](patterns-meta.md#不揃いの石畳) に噛み合わない。

### そこで
**逆向きにも名前を付け、対で置く。**
戻る義務はない。売り場にあるかを問うだけ。
どちらが勝つかは [ザワつく状況](patterns-meta.md#ザワつく状況) が決める。
...
[3ストライクで書く](patterns-meta.md#3ストライクで書く) 前に、枠だけ埋めてしまう。
...
### そこで
**解に反対する力も書く。**
諦めるものまで書けば、[たのしい釣り方](patterns-meta.md#たのしい釣り方) になる。
...
### そこで
**採算で並べ、ときめきで選ぶ。**
諦めた採算は [ザワつく状況](patterns-meta.md#ザワつく状況) に書く。
独りよがりは [3ストライクで書く](patterns-meta.md#3ストライクで書く) が落とす。

![ときめく解法](diagrams/patterns-meta/ときめく解法.svg)

<!--source-->
名前は近藤麻理恵『人生がときめく片づけの魔法』(2011)＝捨てるものではなく残すものを、触ってときめくかで選ぶ。感覚で判定する先例は Alexander の mirror-of-the-self（The Nature of Order, 2002）だが、あちらは個人の好みを超えた一致を測る。書き手のときめきで採れとは誰も言っておらず、そこはこのデッキの追加。
...
[ザワつく状況](patterns-meta.md#ザワつく状況) を伏せ、効いた形だけ渡す。

**完成形を見せると、その形だけが真似される。**
道すじを書いても、退屈なら読み手は辿らない。
辿られない道すじは、結局その形だけを残す。
...
<!--source-->
名前の「たのしい」は高橋征義・後藤裕蔵『たのしいRuby』(2002) の書名から借りた語。内容は Alexander『時を超えた建設の道』(1979)＝パターンは物であると同時に、その物を生成する過程の記述である。／Coplien "A Development Process Generative Pattern Language" (1994)＝生成的パターンは構造に間接的に働きかける。たのしくなければ辿られない、はこのデッキの追加。
...
本文が [たのしい釣り方](patterns-meta.md#たのしい釣り方) と書いても、図が残る。

### そこで
**図はラフのまま出す。示唆にとどめる。**
[ザワつく状況](patterns-meta.md#ザワつく状況) が見えれば、それでよい。
...
辿られないリンクは、[収穫](patterns-wiki.md#収穫) されない。

### そこで
**関連は、本文の文の中に溶かして書く。**
「ここで [不揃いの石畳](patterns-meta.md#不揃いの石畳) が効く」と書けば目に入る。
それが [ジワる名前](patterns-meta.md#ジワる名前) なら、辿らずとも通る。
...
<!--pattern-->
### いつ・なにが困るか
[黙って聴く著者](patterns-meta.md#黙って聴く著者) の場に出す前に、一人で決める。
黙って読むぶんには通るので、それで決めてしまう。

**名前も文も、声に出すまで詰まりが分からない。**
黙読は書き手の頭が補い、どこでも通ってしまう。
分かった頃には他所から張られ、改名すると折れる。

### そこで
**書いた後に、同僚やゴムのアヒルに向けて声に出す。**
詰まったら即、改名する。値段は繋がった数で上がる。
[ジワる名前](patterns-meta.md#ジワる名前) も [文に溶かす](patterns-meta.md#文に溶かす) 一行も、耳で分かる。
...
<!--pattern-->
### いつ・なにが困るか
[3ストライクで書く](patterns-meta.md#3ストライクで書く) を経た原稿を、場に出す。
読者が [声に出して読む](patterns-meta.md#声に出して読む) と、誤読を解きたくなる。
...
<!--pattern-->
### いつ・なにが困るか
1つずつ完成させ、[接ぎ木](patterns-wiki.md#接ぎ木) を後回しにする。
...
### そこで
**上位・同位・下位の3方向へ噛ませる。**
繋ぎ方は [文に溶かす](patterns-meta.md#文に溶かす)。目地は埋めない。
隙間から草が生え、読み手は先へ歩き出す。

![不揃いの石畳](diagrams/patterns-meta/不揃いの石畳.svg)

<!--source-->
Alexander『パタン・ランゲージ』(1977) 序文＝各パターンは、それを含む上位・周りを囲む同位・内に含まれる下位のパターンに支えられてのみ存在する。／名前は同書 247「目地に草の生える石畳」＝目地を1インチ空け、モルタルで固めず地面に直に置く。草や苔はそこから生える。
