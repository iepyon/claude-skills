...
### 基準: [[LLM]]
### 決める: [[ルールベースのプログラミング|ルールベース]] と [[機械学習モデル]]
### 振る舞い: [[ファインチューニング]]
### 材料: [[RAG]] と [[LLM-Wiki|LLM Wiki]]
### 事実: [[データ基盤]]
### 手順: [[Agent-Skills|Agent Skills]]
### 口: [[MCP]]
### 段取り: [[ワークフロー]]
...
<!--pattern-->
### いつ・なにが困るか
同じ手順を、毎回プロンプトに貼り直して頼んでいる。
全部積んでおくと、関係ない指示まで毎回読ませてしまう。

**手順は事実ではないので、[[RAG]] では拾えない。**
「どうやるか」は問いに似ておらず、検索に掛からない。
かといって常時載せると、文脈が指示で埋まっていく。

### そこで
**手順を畳んで置き、要る時だけ開かせる。**
常に載せるのは名前と一行だけ。中身は選ばれてから読む。
足すのは**手順の記憶**。道具の口そのものは [[MCP]]。

![Agent Skills](diagrams/patterns-solutions/Agent-Skills.svg)

<!--source-->
Agent Skills は 2025-12-18 に agentskills.io で公開仕様となり、Agentic AI Foundation が管理する。SKILL.md はフォルダに置く指示書で、YAML の frontmatter（名前と説明）だけが起動時にシステムプロンプトへ載り、本体と付属のファイル・スクリプトは選ばれてから読まれる（progressive disclosure）。2026-06 時点でおよそ40の製品が対応（Claude・OpenAI Codex・GitHub Copilot・VS Code・Cursor・Gemini CLI・Goose ほか）。
