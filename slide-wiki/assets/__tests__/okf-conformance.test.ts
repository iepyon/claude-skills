...
describe("OKF v0.2 §6 リンク", () => {
  ...
  it("旧記法も、内部リンクにならない書き方も残っていない", () => {
    // **この1本は OKF への適合ではなく、この道具の家内規則を見ている。** §6.1 は
    // 相対も絶対も合法とし、絶対形を「推奨」とまで書いているので、`/deck.md#id` を
    // 叱るのは仕様ではなくこちらの決めごとである（理由は ontology.yaml の
    // `inline.internal-link` — GitHub で当たる形を1つに保つため）。
    //
