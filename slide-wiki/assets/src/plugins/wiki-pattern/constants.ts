 ...
 * できて、見た目の重さが釣り合う。
 *
 * **上限は 0.18in。** 左段のテキストの箱は 0.5〜6.0in、下敷きは 6.18in から始まるので、
 * 動かせるのはその 0.18in だけ。ここを超えると、`1行 全角25字`（ontology.yaml の
 * guidance）を使い切った行が図に触る — **字幅がフォントで揺れるぶんの余裕も消える**
 * ので、余りを全部は使わずに 0.03in 残してある。
 */
