...
| `先行` | `後続` | これを済ませてから、指す先へ進む。適用の順序。 | Coplien の generative sequence（"a sequence is one path through a pattern language"） |
| `後続` | `先行` | これのあとに来るパターン。 | 同上 |
| `検算` | 無し（一方向） | 指す先の出来を確かめる打ち手。落ちれば書き直す。 | このバンドルの反復（agenda の `### 検算:` と本文に計6件） |
