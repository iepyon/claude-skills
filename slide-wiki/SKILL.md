...
「読める」の物差しは想定問答（バンドル直下の `questions.yaml`）。アンカーの実在と
検索語の到達可能性はテストが毎回見るが、**問いだけを渡された読み手が実際に答えへ
辿り着くか**は機械では確かめられないので、問いや本文を大きく動かしたら
[docs/answerability-eval.md](docs/answerability-eval.md) の手順でエージェントに解かせて採点する。
