...
2. Gabriel, *A Pattern Language for Writers' Workshops* の **Author Reads Selection**
   ...
   「書いた後に、同僚やゴムのアヒルに向けて声に出す」とは場面が違う。
...
**名前**「魚ではなく釣り方」のほうは出所不明の格言（老子由来と言われるが典拠なし）で、
**そちらは出典に書かない**。書けるのは**「たのしい」の側だけ**で、これは
高橋征義・後藤裕蔵『たのしいRuby』（まつもとゆきひろ監修, 初版 2002）から借りた語。
**借りたのは書名の語だけで、同書はこのパターンを述べていない** — Ruby 入門書であって
パターンの書き方の本ではないので、**内容の出典として並べない**。`メタファ` と同じ扱いで、
読者にとって既に馴染みのある語を借りている（旧名 `釣り竿を渡す` は道具の側を指していて、
渡した道具が使われるかどうかを言えていなかった）。

**「たのしくなければ辿られない」はどの出典にも無い。** Alexander と Coplien が負うのは
「解ではなく解を生成する過程を書く」ところまでで、**その過程が読み手に辿られるかどうかは
論じていない**。だから出典行では**デッキの追加として明示する**（`ジワる名前` の遅効性、
`ときめく解法` の書き手のときめきと同じ扱い）。

**`ときめく解法` に畳まない** — 退ける形は似ているが、測る相手が違う。
`ときめく解法` は**書き手**が自分のときめきで解を選ぶ話（選ぶ時点）。こちらは**読み手**が
渡された道すじを辿るかどうかの話（渡した後）。層が違うので別の一枚で立てる。

> 草案: `名前の「たのしい」は『たのしいRuby』(2002) / 内容は Alexander の生成性 (The Timeless Way of Building, 1979) / Coplien, generative pattern (1994) / 辿られなければ道すじは形に戻る、はデッキの追加`
...
| たのしい釣り方 | ○ | 名前の「たのしい」は高橋征義・後藤裕蔵『たのしいRuby』(2002) / 内容は Alexander『時を超えた建設の道』(1979) の生成性 / Coplien, generative pattern (1994) / 辿られなければ道すじは形に戻る、はデッキの追加 | **名前の由来のうち、書けるのは「たのしい」だけ。**「魚ではなく釣り方」の格言は典拠が無いので書かない。**『たのしいRuby』を内容の出典に並べない** — Ruby 入門書であって、借りたのは書名の語だけ（`メタファ` と同じ、読者に馴染みのある語の借用）。**「たのしくなければ辿られない」を Alexander / Coplien に負わせない** — 両者が言うのは「解を生成する過程を書く」までで、辿られるかは論じていない。**`ときめく解法` に畳まない** — あちらは書き手が選ぶ時点、こちらは読み手が渡された後。**旧名は `釣り竿を渡す`**（道具の側を指していて、渡した道具が使われるかを言えていなかった）|
...
- `たのしい釣り方` の書誌 — 『たのしいRuby』の著者・監修・初版年（高橋征義・後藤裕蔵、
  まつもとゆきひろ監修、2002）を現物で確認する。**借りたのは書名の語だけ**なので
  逐語は要らないが、刻むのは書誌なので版元と刊行年は突き合わせる。
  それまで**刻む文字列は変えない**
...
- `たのしい釣り方` の対（魚も見せる）→ 「いつ完成形を見せるか」の線引きが微妙で、
  真正面から `たのしい釣り方` を打ち消して読まれる危険がある。急がない
