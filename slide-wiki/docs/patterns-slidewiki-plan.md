...
ブランチ `claude/slide-wiki-skill-pattern-c0z5ms` の作業記録。
「wiki パターンではなく slidewiki パターンを整理し直す」という依頼を、
**新デッキ `assets/doc/wiki/patterns-slidewiki.md`（スライドが育つパターン）の新設**として
実装した。書式は patterns-meta と過去の git log の慣例に従い、全枚出典付き。
...
- **intro-slidewiki（第1部）**: 背景と物語。intro-meta が後から足された前例
  （#59, `a38abc2`）に従い、order.yaml の「スライドを育てる」グループの先頭に挿す。
